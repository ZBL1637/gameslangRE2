...
export const MetaphorChallenge: React.FC<MetaphorChallengeProps> = ({
  ...
  const getConnectionStatus = (sourceId: string, targetId: string) => {
    const conn = connections.find(c => c.sourceId === sourceId || c.targetId === targetId);
    if (!conn) return 'none';
    if (conn.sourceId === sourceId && conn.targetId === targetId) return 'connected';
    return 'none';
  };
  ...
  const isConnectionCorrect = (sourceId: string, targetId: string) => {
    ...
    return target?.correctSource === sourceId;
  };
