...
export const GachaMinigame: React.FC<GachaMinigameProps> = ({ onComplete }) => {
  ...
  const [currentTenPull, setCurrentTenPull] = useState<GachaResult[]>([]);
    ...
    <div className="gacha-minigame">
      {!showSummary ? (
        <>
          ...
          {/* 抽卡结果展示 */}
          <div className={`pull-result-area ${currentTenPull.length > 0 ? 'ten-pull-mode' : ''}`}>
            {isAnimating ? (
            ...
            ) : currentTenPull.length > 0 ? (
              <div className="ten-pull-grid">
                {currentTenPull.map((result, index) => (
                  <div key={index} className={`result-card mini ${result.rarity}`} style={{ animationDelay: `${index * 0.1}s` }}>
                    <div className="rarity-badge">{getRarityName(result.rarity)}</div>
                    <h4 className="term-name">{result.term}</h4>
                  </div>
                ))}
              </div>
            ) : currentPull ? (
