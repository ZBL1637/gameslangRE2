...
export const ChartSunburst: React.FC<ChartSunburstProps> = ({ onSelectFragment }) => {
  ...
  const coloredChartData = useMemo(() => {
    return chartData.map((node, idx) => {
      const fallbackColor = PALETTE[idx % PALETTE.length];
      return {
        ...node,
        itemStyle: { ...(node.itemStyle ?? {}), color: node.itemStyle?.color ?? fallbackColor }
      };
    });
  }, [chartData]);
      ...
      series: {
        ...
        levels: [
          ...
          },
          {
            r0: '40%',
            r: '70%',
            colorSaturation: [0.26, 0.7],
            itemStyle: { opacity: 0.85 }
          },
          {
            r0: '70%',
            r: '88%',
            colorSaturation: [0.18, 0.62],
  ...
  return (
    <div ref={containerRef}>
      {!inView ? (
        ...
        <div style={{ color: '#A9B7D6', padding: '12px 0' }}>数据为空或加载失败</div>
      ) : (
